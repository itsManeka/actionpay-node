  ...
  async deeplink(url: string, offer: number, source: number): Promise<any> {
    ...
    try {
      ...
      if (content.error) {
        ...
        for(var key2 in content.result.links){
          if(/url\=/img.test(content.result.links[key2].url)) {
              return content.result.links[key2].url.replace("example.com", encodeURIComponent(url));
          }
        }
